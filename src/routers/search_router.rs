...
LIMIT 30;
            "#;
                let stmt = store
                    ...
                    .await
                    .expect("Failed to prepare only bounding box SQL query");

                let rows = store
                    .client
                    .query(&stmt, &[&minLon, &minLat, &maxLon, &maxLat, &search_term])
                    .await
                    .unwrap();
...
LIMIT 30;
        ...
        let stmt = store
            ...
            .await
            .expect("Failed to prepare some bounding box SQL query");
